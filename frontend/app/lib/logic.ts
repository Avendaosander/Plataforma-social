...
export const getTimeElapsed = (createdAt: any): string => {
  const createdAtMs = typeof createdAt === 'number' ? createdAt : Number(createdAt);
  ...
  if (diff < minute) {
    ...
    if (currentYear === createdAtYear) {
      return `${createdAtDate.getDate()} de ${createdAtDate.toLocaleString('es', { month: 'long' })}`;
    } else {
      // Si es de un año anterior
      return `${createdAtDate.getDate()} de ${createdAtDate.toLocaleString('es', { month: 'long' })} de ${createdAtYear}`;
    }
  }
};
